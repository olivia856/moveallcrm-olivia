...
function renderMasterView(jobs) {
    ...
    tbody.innerHTML = page.map(j => {
        ...
        return `<tr data-id="${j.id}">
            ...
            ${isAdmin() ? `<td style="max-width:180px;white-space:normal;font-size:0.75rem;color:var(--text-secondary);cursor:pointer;" onclick="openPublicComment(${j.id})" title="Click to edit public comment">${escapeHtml(j.public_comment || '—')}</td>` : ''}
            <td class="actions-cell" style="white-space:nowrap; position:relative;">
                ...
                <div id="job-menu-${j.id}" class="job-action-menu hidden" style="position:absolute;right:10px;top:100%;background:var(--bg-elevated);border:1px solid var(--border-color);border-radius:6px;z-index:99;display:flex;flex-direction:column;min-width:160px;box-shadow:var(--shadow-md);">
                    ...
                    ${currentJobsView === 'past' || currentJobsView === 'archived' ? `<button class="btn btn-ghost" style="justify-content:flex-start;border-radius:0;border-bottom:1px solid var(--border-color);padding:10px 16px;" onclick="markJobUpcoming(${j.id})">Move to Master view - upcoming</button>` : ''}
